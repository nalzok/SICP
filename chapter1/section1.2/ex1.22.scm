...
(define (search-for-primes from to)
  (cond ((> from to) 0)
        ((even? from) ; Even numbers are definitely not prime
                      ; so let's skip them for efficiency.
...
; Computers are so powerful nowadays that the time consumed is too short to
; be calculated. I've tested some larger integers to make the timing data 
; significant enough:
;
; 1000000007 *** .03
...
;    I think the sqrt(n) prediction is supported quite well.
;
